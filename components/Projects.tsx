  ...
  <TiltCard className={`glass shape-card ${shapeClass} card-glass border-beam-wrap group flex flex-col h-full ${featured ? 'bento-featured' : ''}`}>
    <div className={`p-6 flex-1 flex flex-col ${featured ? 'md:p-8' : ''}`}>
      <div className="flex justify-between items-start mb-4">
        ...
        {project.link && (
          <a href={project.link} target="_blank" rel="noreferrer" className="p-2 shape-social glass text-zinc-500 hover:text-brand-300 transition-colors duration-300 flex items-center justify-center w-9 h-9">
            <ArrowUpRight className="w-4 h-4" />
          </a>
        )}
      </div>
      <h3 className={`font-display font-semibold text-zinc-50 mb-2 group-hover:text-brand-300 transition-colors duration-300 ${featured ? 'text-2xl md:text-3xl' : 'text-lg'}`}>
        {project.title}
      </h3>
