            ...
            <div className={`glass shape-card shape-skill-${idx} card-glass p-6 h-full group`}>
              ...
              <div className="flex flex-wrap gap-1.5">
                {group.skills.map((skill) => (
                  <span
                    key={skill}
                    className="px-2.5 py-1 bg-white/5 text-zinc-400 rounded-md text-xs border border-white/5 hover:text-brand-300 hover:border-brand-500/30 transition-all cursor-default"
                  >
                    {skill}
                  </span>
                ))}
